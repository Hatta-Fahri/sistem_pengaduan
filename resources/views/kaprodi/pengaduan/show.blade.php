...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <nav class="flex text-sm text-gray-500 gap-2 items-center font-medium mb-2">
        <a href="{{ route('kaprodi.dashboard') }}" class="hover:text-polmed-blue transition-colors flex items-center gap-1">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
            Dashboard
        </a>
        <svg class="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
        <a href="{{ route('kaprodi.statistik') }}" class="hover:text-polmed-blue transition-colors">Statistik Laporan</a>
        <svg class="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
        <span class="text-polmed-blue font-bold">Detail #{{ $pengaduan->id }}</span>
    </nav>

    {{-- Badge peringatan: hak akses read-only --}}
                        ...
                        @if ($pengaduan->is_anonymous)
                            <div class="flex items-center gap-3 text-gray-500">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 mt-0.5">Mahasiswa memilih untuk tidak menampilkan identitasnya.</p>
                                </div>
                            </div>
                        @else
                ...
                @else
                    <div class="space-y-0 relative before:absolute before:inset-0 before:ml-4 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-200 before:to-transparent">
                        ...
                        <div class="relative flex items-start justify-between md:justify-normal md:odd:flex-row-reverse group mb-8">
                            <!-- Icon -->
                            <div class="flex items-center justify-center w-8 h-8 rounded-full border-4 border-white {{ $hDot }} shadow-sm shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:-translate-x-1/2 absolute left-0 md:left-1/2 top-1/2 -translate-y-1/2">
                                <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                            </div>
                            
                            <!-- Content -->
                            <div class="w-[calc(100%-3rem)] md:w-[calc(50%-2rem)] ml-8 md:ml-0 bg-white border border-gray-100 p-5 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                                ...
                                @if ($history->catatan)
                                    <div class="bg-gray-50/80 p-3 rounded-xl border border-gray-100 mb-3 text-sm text-gray-600 font-medium">
                                        "{{ $history->catatan }}"
                                    </div>
                                @endif
        ...
        <div class="xl:col-span-1">
            <div class="bg-white rounded-2xl border border-gray-200 shadow-lg shadow-gray-200/50 p-6 sm:p-8 sticky top-6">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <h2 class="text-xl font-bold text-gray-900 tracking-tight">Informasi Status</h2>
                </div>

                {{-- Status Saat Ini --}}
                <div class="mb-5">
                    <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Status Saat Ini</p>
                    <span class="inline-flex items-center px-3.5 py-2 rounded-xl text-sm font-bold {{ $currentBadge }}">
                        {{ $statusLabels[$pengaduan->status] ?? $pengaduan->status }}
                    </span>
                </div>
                ...
                <div class="mb-5 pb-5 border-b border-gray-100">
                    <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Tanggal Pengajuan</p>
                    <p class="text-sm font-semibold text-gray-800">{{ $pengaduan->created_at->format('d M Y, H:i') }} WIB</p>
                </div>

                {{-- Catatan dari Admin (jika ada) --}}
                @if ($pengaduan->catatan_admin)
                <div class="mb-5 pb-5 border-b border-gray-100">
                    <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Catatan Admin</p>
                    <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 text-sm text-gray-700 font-medium leading-relaxed">
                        {{ $pengaduan->catatan_admin }}
                    </div>
                </div>
                @endif

                {{-- Indikator SLA / Overdue --}}
                @if ($pengaduan->is_overdue)
                <div class="mb-5 pb-5 border-b border-gray-100">
                    <div class="flex items-center gap-2 px-4 py-3 bg-red-50 border border-red-200 rounded-xl">
                        ...
                        <p class="text-sm font-bold text-red-700">Pengaduan Terlambat (Overdue)</p>
                    </div>
                    <p class="text-xs text-gray-500 font-medium mt-2">Pengaduan ini melebihi batas {{ \App\Models\Pengaduan::SLA_HARI }} hari tanpa perubahan status.</p>
                </div>
                @endif

                {{-- Tombol Kembali ke Statistik --}}
                <a href="{{ route('kaprodi.statistik') }}"
                   class="w-full flex items-center justify-center gap-2 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-xl text-sm transition-all">
