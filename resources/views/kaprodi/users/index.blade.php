...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div>
            <form method="GET" action="{{ route('kaprodi.users.index') }}" class="relative w-full sm:w-72">
                    ...
                    <tr class="hover:bg-gray-50/80 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-center whitespace-nowrap">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form method="POST" action="{{ route('kaprodi.users.toggle-active', $user) }}" onsubmit="return confirm('{{ $user->isActive() ? 'Yakin ingin menonaktifkan pengguna ini?' : 'Yakin ingin mengaktifkan pengguna ini?' }}')">
                                    @csrf
                                    @method('PATCH')
                                    @if ($user->isActive())
                                        <button type="submit"
