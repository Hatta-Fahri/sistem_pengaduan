...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <a href="{{ route('kaprodi.kategori.create') }}"
                    ...
                    <tr class="hover:bg-gray-50/80 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-center whitespace-nowrap">
                            <div class="flex items-center justify-center gap-2">
                                {{-- Tombol Edit --}}
                                <a href="{{ route('kaprodi.kategori.edit', $kategori) }}"
                                ...
                                {{-- Tombol Hapus --}}
                                <form method="POST" action="{{ route('kaprodi.kategori.destroy', $kategori) }}" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
