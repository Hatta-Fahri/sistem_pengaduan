...
<head>
    ...
    <style>
        ...
        /* ============================================================
           HALAMAN 1 — padding atas untuk kop, bawah untuk footer
           ============================================================ */
        .page-wrap { padding: 128pt 57pt 50pt; }

        /* ============================================================
           HALAMAN 2, 3, … (di LUAR page-wrap, TOP-LEVEL)
           Padding atas: melewati kop fixed
           Padding kiri/kanan: SAMA dengan page-wrap (57pt) — tidak double
           Padding bawah: melewati footer fixed
           ============================================================ */
        .page-cont {
...
{{-- ======================================================================
     PHP: Hitung chunks SEBELUM halaman 1
     FIRST_ROWS = 6 (sangat konservatif — baris bisa tinggi karena wrap teks)
     Kalkulasi: sections I-III ≈ 350pt, 6 baris × 50pt = 300pt → total 650pt
     Page 1 available: 841 - 128 - 50 = 663pt → aman
     ====================================================================== --}}
@php
    $FIRST_ROWS    = 6;   // baris di halaman 1 (konservatif)
    $NEXT_ROWS     = 22;  // baris per halaman lanjutan
...
{{-- ======================================================================
     HALAMAN 1 — Sections I, II, III + awal Table IV (≤ FIRST_ROWS baris)
     ====================================================================== --}}
<div class="page-wrap">
    ...
    <!-- IV. DAFTAR PENGADUAN — bagian halaman 1 (firstChunk) -->
    ...
    <table class="data-table">
        ...
        @if(! $hasNextChunks)
        {{-- Semua data muat di halaman 1 → tampilkan footer total --}}
        <tfoot>
    ...
    @if(! $hasNextChunks)
    {{-- Tidak ada halaman lanjutan → tanda tangan di halaman 1 --}}
    <div class="ttd-wrap">
        <div class="ttd-inner">
...
</div>{{-- /.page-wrap (AKHIR HALAMAN 1) --}}

{{-- ======================================================================
     HALAMAN 2, 3, … — CHUNK LANJUTAN
     PENTING: div.page-cont berada di LUAR page-wrap (top-level)
     → Tidak ada double padding
     → padding: 128pt atas (kop) | 57pt kiri-kanan | 50pt bawah (footer)
     ====================================================================== --}}
@if($hasNextChunks)
@foreach($nextChunks as $chunk)
<div class="page-cont">

    {{-- Tabel dengan header kolom diulang di setiap halaman lanjutan --}}
    <table class="data-table">
        <thead>
            <tr>
                <th style="width:3%">No</th>
        ...
        @if($loop->last)
        {{-- Footer total hanya di chunk terakhir --}}
        <tfoot>
            <tr><td colspan="10" class="td-left">Total: {{ $totalRows }} pengaduan &mdash; {{ $teksPeriode }}</td></tr>
        </tfoot>
        @endif
    ...
    @if($loop->last)
    {{-- Tanda tangan di chunk terakhir —
         padding-bottom 50pt pada .page-cont menjaga tanda tangan
         tidak tertimpa footer fixed --}}
    <div class="ttd-wrap">
        <div class="ttd-inner">
            <div>Medan, {{ now()->locale('id')->isoFormat('D MMMM YYYY') }}</div>
            <div>Koordinator Program Studi</div>
